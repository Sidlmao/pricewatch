...
- **Scraping**: a GitHub Actions cron runs every 15 minutes and checks whatever is due: new
  items and "check now" requests right away, everything else about every 6 hours. Playwright
  handles the JS-heavy sites.
...
- Paste a link (tracking junk like `utm_*`/`fbclid` is stripped, Amazon links are canonicalised)
  ...
  the first check (about 15 minutes). Pasting the same link twice is caught.
...
```
web/index.html  ── supabase-js ──>  Supabase (auth + Postgres, RLS)  <── psycopg ──  check.py (GitHub Actions, every 15 min)
      │                                                                                     │
...
- The app knows the cron minutes (`cronMinutes` in `config.js`, written by `pages.yml`) so it
  ...
  schedule in `check.yml`, change `CRON_MINUTES` in `pages.yml` too.
...
blocked). One check per item every 6 hours is far below anything a store notices. "Check now"
only moves an item to the front of the next run; it can't hammer a store, and a failing item
isn't retried until its normal slot.
