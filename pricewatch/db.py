...
MIGRATIONS = [  # for databases created before a column existed; errors are ignored
...
def due_items(conn, hours: float):
    """Active items never checked, or last checked more than `hours` ago."""
    ...
    return conn.execute("""
        ...
        WHERE i.active = 1 AND (h.last IS NULL OR h.last < ?)
        ORDER BY i.created_at DESC, i.id DESC""", (cutoff,)).fetchall()
...
def set_telegram(conn, user_id, chat_id):
    with tx(conn):
        if conn.execute("SELECT 1 FROM profiles WHERE user_id = ?", (str(user_id),)).fetchone():
            conn.execute("UPDATE profiles SET telegram_chat_id = ? WHERE user_id = ?", (str(chat_id), str(user_id)))
        else:
            conn.execute("INSERT INTO profiles (user_id, telegram_chat_id, created_at) VALUES (?,?,?)",
